    ...
    <section id="stack" className="relative py-14 sm:py-20">
      <div className="mx-auto max-w-7xl px-5 sm:px-8">
          ...
          <div className="rounded-3xl border border-edge bg-panel p-5 sm:p-6">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div className="flex items-center gap-2">
                <Icon name="store" className="h-5 w-5 text-accent" />
                <h3 className="font-display text-lg font-semibold text-strong">
                  Marketplace Experience
                </h3>
              </div>
              <span className="rounded-full border border-brand-500/25 bg-brand-500/10 px-3 py-1 text-xs font-medium text-accent-strong">
            ...
            <div className="mt-4 flex flex-wrap gap-2.5">
              {marketplaces.map((m) => (
                <span
                  key={m}
                  className="group flex items-center gap-2 rounded-xl border border-edge bg-panel-strong px-4 py-2 text-sm font-medium text-soft transition-all hover:border-brand-500/30 hover:bg-brand-500/10 hover:text-strong"
                >
        ...
        <div className="mt-4 grid gap-4 lg:grid-cols-2">
          ...
          <Reveal>
            <div className="h-full rounded-3xl border border-edge bg-panel p-5 sm:p-6">
              <div className="flex items-center gap-2">
                <Icon name="cpu" className="h-5 w-5 text-accent" />
                <h3 className="font-display text-lg font-semibold text-strong">
                  AI-Assisted Capabilities
                </h3>
              </div>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {aiCapabilities.map((c) => (
                  <span
                    key={c}
                    className="rounded-lg border border-edge bg-panel-strong px-2.5 py-1.5 text-xs text-soft"
                  >
                    {c}
                  </span>
                ))}
                  ...
                  <div key={cat.category}>
                    ...
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      {cat.items.map((item) => (
                        <span
                          key={item}
                          className="rounded-md border border-edge bg-panel-strong px-2 py-1 text-[11px] font-medium text-muted"
                        >
                          {item}
                        </span>
                      ))}
